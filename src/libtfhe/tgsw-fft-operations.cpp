...
static LagrangeHalfCPolynomial *xaiMinusOneTable = NULL; /// Precomputed LUT stored as global table

// Precompute LUT: as global table populated with the x^ai-1
EXPORT void precomputeXaiMinusOne(const TGswParams *params) {
  const TLweParams* tlwe_params=params->tlwe_params;
  const int32_t N = tlwe_params->N;

  xaiMinusOneTable = new_LagrangeHalfCPolynomial_array(2*N,N);

  for (int32_t i=0; i < 2*N; i++)
    {
      LagrangeHalfCPolynomialSetXaiMinusOne(xaiMinusOneTable+i,i);
    }
